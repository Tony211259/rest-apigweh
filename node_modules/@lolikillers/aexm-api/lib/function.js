...
function joox(query){
    return new Promise(async(resolve, reject) => {
    axios.get('https://www.useetv.com/livetv/' + query)
        .then(({ data }) => {
            const $ = cheerio.load(data)
            let hasil = [];
            	let t = $('div.col-xs-12 > div.now').find('h4').text()
            	let u = $('div.col-xs-12 > div.now').find('p').text()
            	let p = $('div.col-sm-6').find('img').attr('src')
            	let d = $('div.col-sm-6').find('img').attr('alt')
            	let a = $('#video-player').attr('poster')
                hasil = {
                	tv: d,
                	logo: p,
                	poster: a,
                	schedule: t,
                	time: u,
                	link: `http://gumisasak-vision.online/Dyos-channel/chunklist.m3u8?id=${query}&gumisasakTV=default&sync=0`,
                	link2: `http://usee.galleryiptv.my.id/GaUsah_Di_Share_Tokennya/chunklist.m3u8?id=${query}&bandwidth=default&sync=0`
                }
            resolve(hasil)
            console.log(hasil)
        })
